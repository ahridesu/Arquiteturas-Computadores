...
int main(void) {
	...
	// configuração do led RE0
	LATEbits.LATE0 = 0;		// set valor inicial
	TRISEbits.TRISE0 = 0;
	// final da configuração do led
	volatile int v, media=0; //volatile diz ao compilador para não optimizar variavel
	while(1) {
		LATEbits.LATE0 = 1;
		...
		int *p = (int *) (&ADC1BUF0);
		int i;
		// obrigatório ler valores do buffer antes de nova conversão 
		for(i=0; i<4; i++) {
			media += p[i*4];
		}
		media = media / 4;
